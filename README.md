...
本版本号：`3.0.0-v2`。V2 使用 18 个独立演奏声部与 1 条厅堂环境声，加入按乐器区分的演奏动作、共享节拍、舞台混响和局部色彩特效。
...
右下角声音按钮显示“声”表示声音开启，显示“静”表示静音。重新开启声音时程序会主动恢复浏览器音频；“测试声音”可绕过人物触发播放独立测试音。控制面板的人物识别监视器同时显示音频状态、19 条同步音轨加载数和实时输出电平。
...
- 18 个独立演奏声部和 1 条厅堂环境声均为 44.1 kHz、32 秒同步循环；多声部共用传输时钟、左右舞台声像、早期反射/厅堂混响和动态压缩。
...
src/audio/         19 条同步音轨、空间声像、厅堂混响与总线动态控制
src/ui/            操作面板
...
- V2 已完成短时浏览器压力与完整 E2E；正式展览仍需在目标机器做 60 分钟及 4–8 小时连续运行；
- 视觉与音乐发布前的最终版权和艺术史审查。

V2 实现与验收记录见 [`docs/V2_IMPLEMENTATION_REPORT.md`](docs/V2_IMPLEMENTATION_REPORT.md)；V1 的 Pro 实现报告和 Codex 验收报告保留为历史基线。
