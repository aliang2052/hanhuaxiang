...
基于仓库基线 `ac76d30` 完成的离线全屏互动装置网页。实体摄像头使用本地语义人物模型，模拟摄像头使用可控前景，把人体区域映射到 9×7、共 63 个触发区域；触发状态驱动画面中的汉画像石人物、群像、墨色反馈和同步音轨。
...
1. 模式选择“摄像头”。
2. 来源选择“实体摄像头”。
3. 点击“启动摄像头”；离线 EfficientDet Lite0 会在本机加载，无需离场采集空背景。
4. 人物识别监视器会叠加红色人物 Mask、黄色人物框和识别置信度。只有模型判定为 `person` 的区域才参与触发，普通窗户、地图或光照变化不会因画面变化直接触发。
