...
基于仓库基线 `ac76d30` 完成的离线全屏互动装置网页。实体摄像头使用本地人像分割模型，模拟摄像头使用可控前景，把人体轮廓映射到画面中 63 个实际人物画格；触发状态驱动画面中的汉画像石单人动画、墨色反馈和同步音轨。
...
右下角声音按钮显示“声”表示声音开启，显示“静”表示静音。重新开启声音时程序会主动恢复浏览器音频；“测试声音”可绕过人物触发播放独立测试音。控制面板的人物识别监视器同时显示音频状态、9 条音轨加载数和实时输出电平。
...
- 63 个视觉节点只使用 8 类干净的单人人物素材，每一格独立裁切、独立动画；运行时会按人物实际不透明轮廓裁掉 PNG 透明留白，再依据画格宽高放大。壁画双人、群像和带陪衬元素的切图仍保留在完整素材目录中，但不进入主画面，避免一格多人或跨格。
