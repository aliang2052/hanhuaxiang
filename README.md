...
`assets/audio-auditions/` 是用户指定的本地试听运行副本，映射见 `config/audio-overrides.json`。试听册自身说明其商业 Demo 仅供本机选音，因此对外发布或重新分发本项目之前，必须另行确认这 36 段声音的授权；原有 27 段 VCSL 音频仍为 CC0。
...
src/ui/            操作面板
assets/source-highres/  干净石纹背景、32 个独立高分辨率人物源和批量设计母版
...
- V4 Motion 仍需在目标机器完成 60 分钟及 4–8 小时连续运行和真实音箱试听；
- 视觉与音乐发布前的最终版权和艺术史审查。
