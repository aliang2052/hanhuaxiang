...
| 场景节点 | 63 | V2 实际使用 47 个不同文件：46 个单人节点、4 个双人节点、10 个三人节点、3 个中央大型群像节点 |
| 离线音频 | 19 | 18 个独立演奏声部 + 1 条厅堂环境声，44.1 kHz / 32 秒 |
...
全部 WAV 由项目内 `tools/build_assets.py` 使用 NumPy 确定性合成，不包含从参考视频、唱片或第三方采样库中截取的音频。18 个演奏声部为：琴、横笛、琵琶、编钟、弓弦、鼍鼓、袖舞、仪礼、笙、排箫、瑟、拍板、铙钹、节铃、扁鼓、石磬、建鼓和角；另有一条厅堂环境声。

这些声音是面向互动装置的原创物理感近似，不应宣传为真实出土汉代乐器录音。浏览器运行时增加共享传输时钟、舞台声像、确定性厅堂脉冲响应和动态压缩；不发起网络请求。
