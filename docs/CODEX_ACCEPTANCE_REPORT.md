...
- JavaScript 语法：38 files passed。
- Node 单元测试：26 passed（含鼠标真实画格命中、音频恢复状态与输出电平测试）。
- Server 测试：2 passed。
...
- 浏览器 E2E：36 + 10 + 11 + 8 = 65 项行为检查全部通过。
...
- 切换输入模式时清除上一模式的激活残留；
- 多点触摸继续使用邻近身体面积覆盖；
- 摄像头、Homography 与背景分割逻辑不变。

独立浏览器回归结果：鼠标测试点命中 ID `28`，`positiveCoverageCount = 1`、`activeIds = [28]`、`pointerTargetIds = [28]`。加入识别与声音诊断后，完整 E2E 为 36 + 10 + 11 + 8 = 65 项全部通过，外部请求和页面/控制台错误为 0。

前述 60 分钟长稳发生在后续鼠标精准、人物识别监视器和声音诊断修正之前；这些后续改动已完成完整单元、服务和浏览器 E2E 回归，但没有把旧长稳伪装成修改后的重新执行结果。

## 7. 用户验收后的声音状态修正

用户启动实体摄像头后反馈无声。后续现场截图确认静音未勾选，但页面始终为 `0 / 63`：摄像头只有原始画面，尚未采集空场背景，因此候选人物、前景像素和触发格子均为零，人物声部不会启动。浏览器诊断同时确认 AudioContext 为 `running`、9 / 9 音轨已加载；Mac 输出音量为 63%、系统未静音，9 个 WAV 也均有非零电平。
...
- 控制面板增加常驻人物识别监视器，实时显示摄像头、前景 Mask、候选人物框、前景像素、触发格数与音频链路；
- 空场采集增加 3 秒倒计时，允许操作者点击后离开摄像头画面。

摄像头模式仍须在无人画面下先完成“采集空场背景”；监视器中的候选人物、前景像素和触发格子变为非零后，人物声部才会被触发。物理扬声器最终是否可听仍需操作者点击“测试声音”现场确认；自动化只能验证浏览器内信号链路，不能替代人的听觉验收。
