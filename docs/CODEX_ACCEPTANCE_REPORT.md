...
- Node 单元测试：29 passed（含鼠标真实画格命中、音频恢复状态、输出电平和人物类别过滤测试）。
- Server 测试：2 passed。
...
独立浏览器回归结果：鼠标测试点命中 ID `28`，`positiveCoverageCount = 1`、`activeIds = [28]`、`pointerTargetIds = [28]`。加入语义人物识别与声音诊断后，完整 E2E 为 36 + 10 + 10 + 8 = 64 项全部通过，外部请求和页面/控制台错误为 0。
...
- 控制面板增加常驻人物识别监视器，实时显示摄像头、人物 Mask、人物框、识别置信度、人物像素、触发格数与音频链路；
- 实体摄像头改用本机 EfficientDet Lite0 语义检测，只接受 `person` 类别，不再要求离场采集空背景；模拟摄像头仍保留可重复验收的空场流程。
